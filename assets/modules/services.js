...
function getCache(key) {
  try {
    ...
    console.warn(
      `Cache corruption detected for key: ${key}. Removing invalid cache.`,
    );
...
function isCacheValid(cacheObj, ttl) {
  return (
    cacheObj && cacheObj.timestamp && Date.now() - cacheObj.timestamp < ttl
  );
}
...
async function fetchServicesData() {
  ...
  if (!servicesRequestPromise) {
    servicesRequestPromise = api.getServices().finally(() => {
      servicesRequestPromise = null;
    });
  }
...
async function fetchCategoriesData() {
  if (categoriesMemoryCache)
    return { data: categoriesMemoryCache, isStale: false };
  ...
  if (!categoriesRequestPromise) {
    categoriesRequestPromise = api.getCategories().finally(() => {
...
async function fetchNotificationsData() {
  if (notificationsMemoryCache)
    return { data: notificationsMemoryCache, isStale: false };
  ...
  if (!notificationsRequestPromise) {
    notificationsRequestPromise = api.getNotifications().finally(() => {
...
function setupAnnouncementScroller(notifications) {
  ...
  const existingScroller = document.querySelector(
  ...
  const scrollerHTML = `
        <div class="card announcements-scroller-card">
            ...
            <div class="announcements-scroller-container">
                ${notifications
                  .map((n) => {
                    const { text, mediaHtml } = parseMediaFromMessage(
                      n.message,
                    );
                    return `
  ...
  try {
    ...
    const runFilters = () => {
      ...
      filteredServices = allServices.filter((s) => {
        ...
        const matchesCategoryName = s.category
          ?.toLowerCase()
          .includes(searchTerm);
      ...
      if (platformBtns) {
        platformBtns.forEach((btn) => {
          ...
          if (currentCategoryFilter === "all" && btnCat === "all") {
            ...
            currentCategoryFilter.toLowerCase().includes(btnCat.toLowerCase())
          ) {
            btn.classList.add("active");
          } else {
    ...
    if (servicesRes.isStale)
      backgroundPromises.push(refreshServicesInBackground());
    if (categoriesRes.isStale)
      backgroundPromises.push(refreshCategoriesInBackground());
    if (notifRes.isStale)
      backgroundPromises.push(refreshNotificationsInBackground());
    ...
    grid.innerHTML =
      '<p class="text-muted text-center">Failed to load services. Please try again later.</p>';
